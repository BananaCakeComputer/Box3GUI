...
class Box3GUI {
    ...
    enterPublicGUI(){
        ...
        this.entity.player.cameraPosition = new GameVector3(6.8, 4, 6.5)
        this.entity.player.cameraTarget = new GameVector3(1, 4, 6.5)
    }
    exitPublicGUI(){
        if(this.entity=='world'){
            world.say('无法给世界使用GUI')
    ...
    publicBackground(meshID){
        if(guiBackground == 0){
            guiBackground = world.createEntity({
                mesh: meshID,
                position:new GameVector3(2,4,6.5),
                meshScale:new GameVector3(0.065, 0.065, 0.065),
                collides:false,
                fixed:true,
                gravity:false,
            })
            ...
            guiBackground = world.createEntity({
                mesh: meshID,
                position:new GameVector3(2.1,4,6.5),
                meshScale:new GameVector3(0.065, 0.065, 0.065),
                collides:false,
                fixed:true,
                gravity:false,
    ...
    publicItemAdd(name, position, meshID, type){
        guiPublicItems[name] = [
            ...
            world.createEntity({
                mesh: meshID,
                position:new GameVector3(2.1,5-(position[1]/16),10-(position[0]/16)),
                meshScale:new GameVector3(0.065, 0.065, 0.065),
                collides:false,
                fixed:true,
                gravity:false,
    ...
    publicItemDelete(name){
        ...
        for(var i = 0;i < guiPublicItemsNames.length;i++){
            if(guiPublicItemsNames[i] == name){
                guiPublicItemsNames.splice(i,1)
                return
            }
        }
    }
    ...
    publicItemScale(item,x,y){
        guiPublicItems[item][1].meshScale = new GameVector3(0.065, y*0.065, x*0.065);
    }
    publicTint(color){
        for(var z = 2;z <= 9;z++){
    ...
    world.onTick(()=>{
        ...
        if(entity.guiBtnI==0 && guiPublicItems[guiPublicItemsNames[0]][2]==='img'){
            ...
            for(var i = 0;i <= (guiPublicItemsNames.length-1);i++){
                guiPublicItems[guiPublicItemsNames[i]][1].meshColor = new GameRGBAColor(1, 1, 1, 1)
            }
            publicHasBtn = false
            for(var i = 0;i <= (guiPublicItemsNames.length-1);i++){
            ...
            if(!entity.buttonPress){
                guiPublicItems[guiPublicItemsNames[entity.guiBtnI]][1].meshColor = new GameRGBAColor(1, 1, 0, 1)
            }else{
                guiPublicItems[guiPublicItemsNames[entity.guiBtnI]][1].meshColor = new GameRGBAColor(0.7, 0.7, 0, 1)
            }
        }
    })
})
...
world.onPlayerLeave(({entity}) => {
    for(let i in guiSpace){
        if(guiSpace[i]==entity.player.name){
            guiSpace[i] = false
            break
        }
    }
